...
for row in rows:
    ...
    for name in names:
        if country in name.text:
            print(f'\n{menu[0]}: {name.text}')
            print(f'{menu[1]}: {name.find_next_sibling("td").text}')
            print(f'{menu[2]}: {name.find_next_sibling("td").find_next_sibling("td").text}')
            print(f'{menu[3]}: {name.find_next_sibling("td").find_next_sibling("td").find_next_sibling("td").text}')